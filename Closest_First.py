...
def getStops(granul):
    ...
    condRO = np.logical_and(pixels[:, :, 2] > color1[0]-10, pixels[:, :, 2] < color1[0]+30)
    condGO = np.logical_and(pixels[:, :, 1] > color1[1]-10, pixels[:, :, 1] < color1[1]+30)
    condBO = np.logical_and(pixels[:, :, 0] > color1[2]-10, pixels[:, :, 0] < color1[2]+30)
    cond1O = np.logical_and(condRO, condGO)
    condOrange = np.logical_and(cond1O, condBO)


    condRY = np.logical_and(pixels[:, :, 2] > color2[0]-10, pixels[:, :, 2] < color2[0]+30)
    condGY = np.logical_and(pixels[:, :, 1] > color2[1]-10, pixels[:, :, 1] < color2[1]+30)
    condBY = np.logical_and(pixels[:, :, 0] > color2[2]-10, pixels[:, :, 0] < color2[2]+30)
...
fig = pylab.figure(figsize=(4.5, 4.0), dpi=100)
ax = fig.gca()
...
while running:
    ...
    if graphVar == 0:
        ...
        ax.set_ylabel("Frequency")
    if graphVar == 2:
        normWaitTravelTimes = pd.DataFrame(list(zip(normWaitTimes, normTravelTimes)), columns=["Waiting Time", "Travel Time"])
    ...
    baveragetext = str("Average Waiting Time: " + str(np.round(np.mean(waitTimes), 2)) + ", Average Travel time: " + str(np.round(np.mean(travelTimes), 2)) + ", Average Travel time: " + str(np.round(np.mean(totalTimes), 2)))
