    ...
    def assignBus(self, passenger):
        if passenger.start.id != -1:
            ...
            for bus in buses:
                if not done:
                    if bus.plan == []:
                        bus.plan.append(passenger.start.id)
                        bus.plan.append(passenger.end.id)
                        bus.assignedPass.append(passenger)
                        bus.newPass = True
                        passenger.bus = bus
                        done = True
            if not done:
                ...
                for bus in buses:
                    routeLen = 0
                    for i in range(len(bus.plan)):
                        if i < len(bus.plan)-1:
                            routeLen += dijkstra[bus.plan[i]][bus.plan[i+1]]
                    dist = 100000
                    for onStop in range(len(bus.plan)):
                        tempOn = dijkstra[passenger.start.id][bus.plan[onStop-1]] + dijkstra[passenger.end.id][bus.plan[onStop]]
                        for offStop in range(len(bus.plan[onStop:])):
                            tempOff = dijkstra[passenger.start.id][bus.plan[onStop-1]] + dijkstra[passenger.end.id][bus.plan[onStop]]
                            temp = tempOn + tempOff
                            if temp + routeLen < dist:
                                if temp + routeLen < MAXDIST:
                                    dist = temp
                                    bestOn = onStop
                                    bestOff = offStop
                    if dist < bestTotal:
                ...
                if bestBus is not None:
                    bestBus.plan.insert(bestOn+1, passenger.start.id)
                    bestBus.plan.insert(bestOff+1, passenger.end.id)
                    bestBus.assignedPass.append(passenger)
                    bestBus.newPass = True
                    controlText = "Passenger " + str(passenger.id) + " has been assigned bus " + str(bestBus.id)
                    self.log = font.render(controlText, True, (0, 0, 0), (255, 255, 255))
                    self.newLog = True
                passenger.bus = bestBus
