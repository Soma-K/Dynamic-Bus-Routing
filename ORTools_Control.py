...
class controlCentre(pygame.sprite.Sprite):
        ...
        for passenger in passengers:
            if passenger.start.id != -1:
                if not passenger.onBus:
                    ...
                    fullStops.append(passenger.bus.nextStop.id)
                    fullStops.append(passenger.end.id)
                    passengerData[passenger.bus.nextStop.id] = 0
                    passengerData[passenger.end.id] = 0
                    pickDeliver.append([passenger.bus.nextStop.id, passenger.end.id])
                    orderedPass.append(passenger.bus.nextStop.id)
                    orderedPass.append(passenger.end.id)
        orderedPass = list(set(orderedPass))
        orderedPass.sort()
    ...
    def getRoute(self):
        ...
        self.search_parameters.time_limit.seconds = 15
    ...
    def update(self):
        if self.newPass:
            ...
            for bus in buses:
                bus.plan = []
                bus.plan.append(bus.id)
                if self.lastSol is not None and self.data is not None:
                    for i in self.lastSol[bus.id][1:-1]:
                        bus.plan.append(list(self.data['passengerData'].keys())[list(self.data['passengerData'].values()).index(i)])
                bus.plan.append(0)
                for passenger in passengers:
                    if passenger.start.id in bus.plan:
                        passenger.bus = bus
            self.newLog = True
            self.calctimes.append(np.round(time.time()-start_time, 2))
            cText = "After new passenger request route calculation took " + str(self.calctimes[-1]) + " seconds"
            self.log = font.render(cText, True, (0, 0, 0), (255, 255, 255))
            self.newPass = False
